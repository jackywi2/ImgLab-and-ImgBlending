...
class MyWidget(QtWidgets.QWidget):
    ...
    def loadLabel(self):
        ...
        filePath , filetype = QtWidgets.QFileDialog.getOpenFileName(directory='dataset',filter='TXT(' + imgfilePath.split('/')[-1][:-4] + '.txt)')
        if filePath:
            ret = self.mbox.question(self, 'question', 'New BoundingBox Label File?',self.mbox.StandardButton.Cancel,self.mbox.StandardButton.Ok)
            if ret == self.mbox.StandardButton.Ok:
                try:
                    with open(filePath) as file_obj:
                        ...
                        for line in file_obj:
                            ...
                            x1 = int(x*image_width - w*image_width/2)
                            y1 = int(y*image_height - h*image_height/2)
                            x2 = int(x*image_width + w*image_width/2)
                            y2 = int(y*image_height + h*image_height/2)
                            obj_name = object_list[obj]
                    ...
                    self.mbox = QtWidgets.QMessageBox(self)
                    self.mbox.setText('This file is not in YOLO label format!')   # 通知文字
                    self.mbox.setIcon(QtWidgets.QMessageBox.Icon.Critical)  # 加入問號 icon
                    self.mbox.exec()
...
class inputWindow(QtWidgets.QWidget):
    ...
    def clearAll(self):
        ...
        try:
            self.mbox = QtWidgets.QMessageBox(self)
            ret = self.mbox.question(self, 'question', 'Delete all class name?',self.mbox.StandardButton.Cancel,self.mbox.StandardButton.Ok)
            if ret == self.mbox.StandardButton.Ok:
                self.object.clear()
                self.listwidget.clear()
                object_list.clear()
